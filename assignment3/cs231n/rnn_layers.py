...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        ...
        if t == 0:
            dx_t, dh0, dWx_t, dWh_t, db_t = rnn_step_backward(dcurrent_h, cache[t])
        else:
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    temp = np.tanh(next_c)
    next_h = o*temp
    cache = (x,Wx,Wh,i,f,o,g,prev_c,next_c,prev_h,next_h,temp)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x,Wx,Wh,i,f,o,g,prev_c,next_c,prev_h,next_h,temp = cache
    N, H = prev_h.shape
    
    do = np.tanh(next_c)*dnext_h
    dtemp = o*dnext_h
    dnext_c += (1-temp**2)*dtemp # Note += as adding to upstream gradient
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1,-1,-1):
        ...
        if t == 0:
            dx_t, dh0, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dcurrent_h, dc, cache[t])
        else:
            dx_t, dprev_h, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dcurrent_h, dc, cache[t])
